...
class KnowledgePoint3Solution(SolutionGenerator):
    def get_solution(self, template: str, args: dict) -> str:
        def get_tens_statement(num):
            if num == 0:
                return "Since the result is 0, we can ignore 0 in tens place."
            else:
                return f"Write down the {num} in the tens place."
    ...
    def get_solution(self, template: str, args: dict) -> str:
        ...
        def check_class_name(idx, arg1, arg2):
            if arg1 == idx or arg2 == idx:
                return '''class="number-line-circle"'''
            else:
                return ''
...
class KnowledgePoint7Solution(SolutionGenerator):
    def get_solution(self, template: str, args: dict) -> str:
        def statement_of_one_tens(arg1):
            if (arg1 % 10 == 0):
                return 'In this case, because there are no ones in {}, the "__ones" part of the question is 0, and we can also simplify the question to "{} = {} tens".'.format(
                    arg1, arg1, arg1 // 10)
            else:
                return ''
...
class KnowledgePoint9Solution(SolutionGenerator):
    def get_solution(self, template: str, args: dict) -> str:
        ...
        def sub_statment1(num):
            if (num < 10):
                return '{} has 1 digit and We put {} in the ones place'.format(num, num)
            return '{} has digits in both the ones and tens places, so we write {} in the correct columns based on its ones and tens digits'.format(
                num, num)
...
class KnowledgePoint11Solution(SolutionGenerator):
    def get_solution(self, template: str, args: dict) -> str:
        ...
        def generate_statement(arr: list, diff_val: int, missing_idx_list: list):
            ...
            def generate_substatment(arr: list, diff_val: int, missing_idx_list: list):
                step = 3
                sub_statement = ""
                for i in range(1, len(missing_idx_list)):
                    missing_idx = missing_idx_list[i]
                    sub_statement += """
**Step {}:** Now we need to find the number that comes after {}. We know the numbers are going up by {} each time, so the number after {} should be {}.
$$
{} + {{\color{{#8DB600}}{}}} = {}
$$\n""".format(step, arr[missing_idx - 1], diff_val, arr[missing_idx - 1], arr[missing_idx - 1] + diff_val,
               arr[missing_idx - 1], diff_val, arr[missing_idx - 1] + diff_val)
            ...
            if first_occurrence > 0:
                statement = """\n
**Step 2:** We know that the numbers are going up by {} each time, so the number that comes after {} should be {}.
$$
{} + {{\color{{#8DB600}}{}}} = {}
$$\n""".format(diff_val, working_arr[first_occurrence - 1], working_arr[first_occurrence - 1] + diff_val,
               working_arr[first_occurrence - 1],
               diff_val,
               working_arr[first_occurrence - 1] + diff_val)
                working_arr[first_occurrence] = working_arr[first_occurrence - 1] + diff_val
            else:
                statement = """\n
**Step 2:** We know that the numbers are going up by {} each time, so the number that comes before {} should be {}.
$$
{} - {{\color{{#8DB600}}{}}} = {}
$$\n""".format(diff_val, working_arr[first_occurrence + 1], working_arr[first_occurrence + 1] - diff_val,
               working_arr[first_occurrence + 1],
               diff_val, working_arr[first_occurrence + 1] - diff_val)
                working_arr[first_occurrence] = working_arr[first_occurrence + 1] - diff_val
            statement += generate_substatment(working_arr, diff_val, missing_idx_list)
            return statement
        ...
        def missing_statment(arr, idxs):
            if len(idxs) == 1:
                return "So the missing number is " + str(arr[idxs[0]])
            statement = "So the missing numbers are "
            for i in range(len(idxs) - 1):
                statement += str(arr[idxs[i]]) + ", "
            statement = statement[:-2] + " "
            statement += "and " + str(arr[idxs[-1]]) + "."
            return statement
        ...
        missing_numbers = missing_statment(arr, idxs);
...
class KnowledgePoint12Solution(SolutionGenerator):
    def get_solution(self, template: str, args: dict) -> str:
        def generate_one_line(num1: str, num2: str, idx):
            idx_mapping = {0: "The leftmost digit", 1: "The second digit from the left",
                           2: "We compare the third digit from the left"}

            if (num1[idx] == num2[idx]):
                return idx_mapping[idx] + " in both numbers is " + num1[idx] + ", which is the same."
            else:
                return idx_mapping[idx] + " in the first number is " + num1[idx] + ", and the second number is " + num2[
                    idx] + "."

        def sub_statement(num1: str, num2: str) -> str:
            statement = """
Since both numbers have the same number of digits, compare the digits from left to right, taking into account their place value.\n"""
            num_of_digits = SolutionGenerator.count_digits(int(num1));
            prefix_num1 = ""
            prefix_num2 = ""
            for i in range(num_of_digits):
                line = "\n" + "**Step {}:** ".format(i + 1) + generate_one_line(num1, num2, i) + "\n"
                line_conclusion = """
<div align="center">
...
${}{{\color{{#8DB600}}{}}}{}$ ? ${}{{\color{{#8DB600}}{}}}{}$
                ...
                if num1[i] < num2[i]:
                    line_conclusion += "\n" + """
**Step {}:** Since {} is less than {}, we can conclude that the number {} is less than the number {}.
$$
{} {{\color{{Salmon}}\:<\:}} {}
$$""".format(i + 2, num1[i], num2[i], num1, num2, num1, num2)
                elif num1[i] > num2[i]:
                    line_conclusion += """
**Step {}:** Since {} is greater than {}, we can conclude that the number {} is greater than the number {}.
$$
{} {{\color{{Salmon}}\:>\:}} {}
$$""".format(i + 2, num1[i], num2[i], num1, num2, num1, num2)
...
$$""".format(num1, num2, num1, num2)
        ...
        def generate_statement(arg1, arg2):
            ...
            if (digits1 > digits2):
                st = '''Since {} has {} digits, while {} has {} digits, we know that {} is larger than {}. Therefore, the answer  is
$$
{} {{\color{{Salmon}}\:>\:}} {}
$$'''.format(arg1, num2words(digits1), arg2, num2words(digits2), digits1, digits2, arg1, arg2)
                return st

            else:
                st = '''Since {} has {} digits, while {} has {} digits, we know that {} is less than {}. Therefore, the answer  is
$$
{} {{\color{{Salmon}}\:<\:}} {}
$$'''.format(arg1, num2words(digits1), arg2, num2words(digits2), digits1, digits2, arg1, arg2)
...
[[arg1]] [[arg2]] + [[arg3]] [[arg4]] = [[arg1 * value_dict[arg2]]] + [[arg3 * value_dict[arg4]]] = [[arg1 * value_dict[arg2] + arg3 * value_dict[arg4]]]
        ...
        args['value_dict'] = {'tens': 10, 'ones': 1, 'hundreds': 100}
        return self.custom_format(template, **args)
    ...
    def get_solution(self, template: str, args: dict) -> str:
        ...
        def number_word_argument(arg1):
            if str(arg1).isnumeric():
                return f"What is {arg1} in word form? "
            else:
                return f"What number is **{arg1}**"
...
class KnowledgePoint24Solution(SolutionGenerator):
    def get_solution(self, template: str, args: dict) -> str:
        def get_rad_item():
            items = ['apples', 'cupcakes', 'apples', 'Bananas', 'Cookies', 'Books', 'Toys', 'Flowers']
...
We can divide the digit in units place.  
$$  
\begin{array}{r}  
[[arg1//arg2]]\phantom{)}   \\  
...
class KnowledgePoint34Solution(SolutionGenerator):
    def get_solution(self, template: str, args: dict) -> str:
        ...
        def compare_nums_statement(num1, num2):
            if num1 == num2:
                return "First compare the numbers, {} equals {}.  Place the first number as the **multiplicand** and the second number as the **multiplier**.".format(
                    num1, num2)
            if num1 // 10 == num2 // 10:
                return "First compare the numbers, {} is greater than {} **because in units place digits {} is greater than {}**. First place the bigger number as **multiplicand** and then second number as **multiplier**.".format(
                    max(num1, num2), min(num1, num2), max(num1, num2) % 10, min(num1, num2) % 10)
            else:
                return "First compare the numbers, {} is greater than {} **because in tens place digits {} is greater than {}**. First place the bigger number as **multiplicand** and then second number as **multiplier**.".format(
                    max(num1, num2), min(num1, num2), max(num1, num2) // 10, min(num1, num2) // 10)
...
\:\:\:\longrightarrow    \:\:
\begin{{aligned}}
&\underline{{\text{{ }}\:{{\color{{Salmon}} {} }}\text{{ }}\text{{ }}\small\:\:\:}}\\[-5pt]
{}&\big){}\\[-4pt]
\end{{aligned}}
$$
"""
...
$$
\begin{{aligned}}
&\underline{{\text{{ }}\:{}\text{{ }}\text{{ }}\small\:\:\:}}\\[-5pt]
{}&\big){}\\[-4pt]
\end{{aligned}}
\:\:\:\longrightarrow    \:\:
\begin{{aligned}}
...
$$
"""
            step1_statement = (step1_part1.format(arg1 // 100, arg1 // 100 // arg2, arg1 // 100 // arg2,
                                                  arg2, arg1, arg1 // 100 // arg2, arg2, arg1)
                               + step1_part2.format(arg1 // 100 // arg2 * arg2 * 100, arg1 // 100,
                                                    arg1 // 100 // arg2, arg2, arg1, arg1 // 100 // arg2, arg2, arg1,
                                                    arg1 // 100 // arg2 * arg2 * 100)
                               + step1_part3.format(arg1 // 100 // arg2 * arg2 * 100, arg1,
                                                    arg1 // 100 // arg2, arg2, arg1, arg1 // 100 // arg2 * arg2 * 100,
                                                    arg1 // 100 // arg2, arg2, arg1, arg1 // 100 // arg2 * arg2 * 100,
...
\:\:\:\longrightarrow    \:\:
\begin{{aligned}}
&\underline{{\:\:{} {{\color {{Salmon}} {} }}\small\:\:\:}}\\[-5pt]
{}&\big){}\\[-4pt]
&{{\:\:{}\:\:}}\\[-4pt]
\hline
&\text{{ }}\:\:{{\color {{Salmon}} {} }}\\[-4pt]
...
"""
            step2_statement = (
                    step2_part1.format(arg1 % 100 // 10, arg2, arg1 % 100 // 10 // arg2, arg1 % 100 // 10 // arg2,
                                       arg1 % 100 // 10 // arg2,
                                       arg1 // 100 // arg2, arg2, arg1, arg1 // 100 // arg2 * arg2 * 100,
                                       arg1 - arg1 // 100 // arg2 * arg2 * 100,
                                       arg1 // 100 // arg2, arg1 % 100 // 10 // arg2, arg2, arg1,
                                       arg1 // 100 // arg2 * arg2 * 100, arg1 - arg1 // 100 // arg2 * arg2 * 100)
                    + step2_part2.format(arg1 % 100 // 10 // arg2 * arg2 * 10, arg2, arg1 % 100 // 10 // arg2,
                                         arg1 // 10 // arg2, arg2, arg1, arg1 // 100 // arg2 * arg2 * 100,
                                         arg1 - arg1 // 100 // arg2 * arg2 * 100, arg1 // 10 // arg2, arg2, arg1,
                                         arg1 // 100 // arg2 * arg2 * 100, arg1 - arg1 // 100 // arg2 * arg2 * 100,
                                         arg1 % 100 // 10 // arg2 * arg2 * 10,
                                         arg1 // arg2 % 10 * arg2 + arg1 % arg2)
            )

            step3_part1 = r"""**Step 3:**
...
$$
"""
            step3_statement = (step3_part1.format(arg1 // arg2 % 10 * arg2 + arg1 % arg2, arg2,
                                                  arg1 // arg2 % 10 * arg2 + arg1 % arg2, arg2,
                                                  arg1 // 10 // arg2, arg2, arg1, arg1 // 100 // arg2 * arg2 * 100,
                                                  arg1 - arg1 // 100 // arg2 * arg2 * 100,
                                                  arg1 % 100 // 10 // arg2 * arg2 * 10,
                                                  arg1 // arg2 % 10 * arg2 + arg1 % arg2,
                                                  arg1 // 10 // arg2, arg1 // arg2 % 10, arg2, arg1,
                                                  arg1 // 100 // arg2 * arg2 * 100,
                                                  arg1 - arg1 // 100 // arg2 * arg2 * 100,
                                                  arg1 % 100 // 10 // arg2 * arg2 * 10,
                                                  arg1 // arg2 % 10 * arg2 + arg1 % arg2)
                               + step3_part2.format(arg1 // arg2 % 10 * arg2, arg1 // arg2 % 10, arg2,
                                                    arg1 // arg2, arg2, arg1, arg1 // 100 // arg2 * arg2 * 100,
                                                    arg1 - arg1 // 100 // arg2 * arg2 * 100,
                                                    arg1 % 100 // 10 // arg2 * arg2 * 10,
                                                    arg1 // arg2 % 10 * arg2 + arg1 % arg2,
                                                    arg1 // arg2, arg2, arg1, arg1 // 100 // arg2 * arg2 * 100,
                                                    arg1 - arg1 // 100 // arg2 * arg2 * 100,
                                                    arg1 % 100 // 10 // arg2 * arg2 * 10,
                                                    arg1 // arg2 % 10 * arg2 + arg1 % arg2, arg1 // arg2 % 10 * arg2)
                               + step3_part3.format(arg1 // arg2 % 10 * arg2, arg1 // arg2 % 10 * arg2 + arg1 % arg2,
                                                    arg1 // arg2, arg2, arg1, arg1 // 100 // arg2 * arg2 * 100,
                                                    arg1 - arg1 // 100 // arg2 * arg2 * 100,
                                                    arg1 % 100 // 10 // arg2 * arg2 * 10,
                                                    arg1 // arg2 % 10 * arg2 + arg1 % arg2, arg1 // arg2 % 10 * arg2,
...
$$
\begin{{aligned}}
&\underline{{\:\:{} \small\:{{\color{{salmon}} R\small\,{} }}}}\\[-5pt]
{}&\big){}\\[-4pt]
&{{\:\:{}\:\:}}\\[-4pt]
\hline
&\text{{ }}\:\:\:{}\\[-4pt]
...
""".format(arg1, arg2, arg1 % arg2,
           arg1 // arg2, arg1 % arg2, arg2, arg1, arg1 // 100 // arg2 * arg2 * 100,
           arg1 - arg1 // 100 // arg2 * arg2 * 100, arg1 % 100 // 10 // arg2 * arg2 * 10,
           arg1 // arg2 % 10 * arg2 + arg1 % arg2, arg1 // arg2 % 10 * arg2, arg1 % arg2)
...
\end{{aligned}}  
$$
""".format(arg2, arg1, arg2, arg1)

            step1_part1 = r"""
**Step 1:**
The short way to think is this: How many {}s are there in {}? Since the answer is 0, we move on to the next place value.

**Step 1.1:** How many {}s are there in {}? We estimate {} and write it above the tens' place.
$$
\begin{{aligned}}
...
\:\:\:\longrightarrow    \:\:
\begin{{aligned}}
&\underline{{\text{{ }}\:0{{\color{{Salmon}}{}}}\text{{ }}\text{{ }}\small\:\:}}\\[-5pt]
{}&\big){}\\[-4pt]
\end{{aligned}}
$$
"""
            step1_part2 = r"""
**Step 1.2:**  We calculate ${} \times {} = {}$ and write {} below {}.
$$
\begin{{aligned}}
&\underline{{\text{{ }}\:0{}\text{{ }}\text{{ }}\small\:\:}}\\[-5pt]
{}&\big){}\\[-4pt]
...
"""
            step1_part3 = r"""
**Step 1.3:** And we subtract {} from {} and then write {}.
$$
\begin{{aligned}}
&\underline{{\text{{ }}\:0{}\text{{ }}\text{{ }}\small\:\:}}\\[-5pt]
{}&\big){}\\[-4pt]
...
$$
"""
            step1_statement = (step1_part1.format(arg2, arg1 // 100, arg2, arg1 // 10, arg1 // 10 // arg2,
                                                  arg2, arg1, arg1 // 10 // arg2, arg2, arg1)
                               + step1_part2.format(arg2, arg1 // 10 // arg2, arg1 // 10 // arg2 * arg2,
                                                    arg1 // 10 // arg2 * arg2, arg1 // 10,
                                                    arg1 // 10 // arg2, arg2, arg1, arg1 // 10 // arg2, arg2, arg1,
                                                    arg1 // 10 // arg2 * arg2)
                               + step1_part3.format(arg1 // 10 // arg2 * arg2, arg1 // 10,
                                                    arg1 // 10 - arg1 // 10 // arg2 * arg2,
                                                    arg1 // 10 // arg2, arg2, arg1, arg1 // 10 // arg2 * arg2,
                                                    arg1 // 10 // arg2, arg2, arg1,
                                                    arg1 // 10 // arg2 * arg2, arg1 // 10 - arg1 // 10 // arg2 * arg2)
                               )
...
"""
            step2_part2 = r"""
**Step 2.2:** How many {}s are there is {}? We estimate ${}$ and write it above the one's place.
...
\:\:\:\longrightarrow    \:\:
\begin{{aligned}}
&\underline{{\text{{ }}\:0{}{{\color {{Salmon}}{} }}\text{{ }}\small\:}}\\[-5pt]
{}&\big){}\\[-4pt]
&{{\:\:\:\!{}\:\:}}\\[-4pt]
\hline
&\text{{ }}\:\:\:{}\\[-4pt]
...
"""
            step2_part3 = r"""
**Step 2.3:**  We calculate ${} \times {} = {}$ and write {} below {}.
$$
\begin{{aligned}}
&\underline{{\text{{ }}\:0{}{}\text{{ }}\small\:}}\\[-5pt]
{}&\big){}\\[-4pt]
&{{\:\:\:\!{}\:\:}}\\[-4pt]
\hline
&\text{{ }}\:\:\:{}\\[-4pt]
...
"""
            step2_part4 = r"""
**Step 2.4:** And we subtract {} from {} and then write {}.
$$
\begin{{aligned}}
&\underline{{\text{{ }}\:0{}{}\text{{ }}\small\:}}\\[-5pt]
{}&\big){}\\[-4pt]
...
"""
            step2_statement = (
                    step2_part1.format(arg1 // 10 // arg2, arg2, arg1, arg1 // 10 // arg2 * arg2,
                                       arg1 // 10 - arg1 // 10 // arg2 * arg2,
                                       arg1 // 10 // arg2, arg2, arg1 // 10, arg1 % 10, arg1 // 10 // arg2 * arg2,
                                       arg1 // 10 - arg1 // 10 // arg2 * arg2, arg1 % 10)
                    + step2_part2.format(arg2, arg1 - arg1 // 10 // arg2 * arg2 * 10,
                                         (arg1 - arg1 // 10 // arg2 * arg2 * 10) // arg2,
                                         arg1 // 10 // arg2, arg2, arg1, arg1 // 10 // arg2 * arg2,
                                         arg1 - arg1 // 10 // arg2 * arg2 * 10,
                                         arg1 // 10 // arg2, (arg1 - arg1 // 10 // arg2 * arg2 * 10) // arg2, arg2,
                                         arg1, arg1 // 10 // arg2 * arg2, arg1 - arg1 // 10 // arg2 * arg2 * 10)
                    + step2_part3.format(arg2, (arg1 - arg1 // 10 // arg2 * arg2 * 10) // arg2,
                                         arg1 // arg2 % 10 * arg2, arg1 // arg2 % 10 * arg2,
                                         arg1 - arg1 // 10 // arg2 * arg2 * 10,
                                         arg1 // 10 // arg2, (arg1 - arg1 // 10 // arg2 * arg2 * 10) // arg2,
                                         arg2, arg1, arg1 // 10 // arg2 * arg2, arg1 - arg1 // 10 // arg2 * arg2 * 10,
                                         arg1 // 10 // arg2, (arg1 - arg1 // 10 // arg2 * arg2 * 10) // arg2,
                                         arg2, arg1, arg1 // 10 // arg2 * arg2, arg1 - arg1 // 10 // arg2 * arg2 * 10,
                                         arg1 // arg2 % 10 * arg2)
                    + step2_part4.format(arg1 // arg2 % 10 * arg2, arg1 - arg1 // 10 // arg2 * arg2 * 10, arg1 % arg2,
                                         arg1 // 10 // arg2, (arg1 - arg1 // 10 // arg2 * arg2 * 10) // arg2,
                                         arg2, arg1, arg1 // 10 // arg2 * arg2, arg1 - arg1 // 10 // arg2 * arg2 * 10,
                                         arg1 // arg2 % 10 * arg2,
                                         arg1 // 10 // arg2, (arg1 - arg1 // 10 // arg2 * arg2 * 10) // arg2,
            ...
            step3_statement = r"""**Step 3: Write the remainder near the quotient and check**
$$
\begin{{aligned}}
&\underline{{\text{{ }}\:\:\:{}\small\:{{\color{{salmon}}R\small\,{} }}}}\\[-5pt]
{}&\big){}\\[-4pt]
&{{\:\:\:\!{}\:\:}}\\[-4pt]
\hline
&\text{{ }}\:\:\:{}\\[-4pt]
...
\end{{aligned}}
$$
""".format(arg1 // arg2, arg1 % arg2, arg2, arg1, arg1 // 10 // arg2 * arg2, arg1 - arg1 // 10 // arg2 * arg2 * 10,
           arg1 // arg2 % 10 * arg2,
           arg1 % arg2)
...
\:\:\:\longrightarrow    \:\:
\begin{{aligned}}
&\underline{{\text{{ }}\:00{{\color {{Salmon}}{}}}\text{{ }}\text{{ }}\:\:\small\:\:\:}}\\[-5pt]
{}&\big){}\\[-4pt]
\end{{aligned}}
$$
"""
...
\:\:\:\longrightarrow    \:\:
\begin{{aligned}}
&\underline{{\text{{ }}\:00{}\text{{ }}\:\:\small\:\:\:}}\\[-5pt]
{}&\big){}\\[-4pt]
-&{{\:\:\:\:{{\color {{Salmon}}{}}}\:\:}}\\[-4pt]
\hline
\end{{aligned}}
...
"""
            step2_part3 = r"""
**Step 2.3:** And we subtract {} from {} and then write {}.
$$
\begin{{aligned}}
&\underline{{\text{{ }}\:00{}\text{{ }}\:\:\small\:\:\:}}\\[-5pt]
{}&\big){}\\[-4pt]
...
$$
            """
            step2_statement = (step2_part1.format(arg2, arg1 // 10, arg1 // 10 // arg2,
                                                  arg2, arg1, arg1 // 10 // arg2, arg2, arg1)
                               + step2_part2.format(arg1 // 10 // arg2, arg2, arg1 // 10 // arg2 * arg2,
                                                    arg1 // 10 // arg2 * arg2, arg1 // 10,
                                                    arg1 // 10 // arg2, arg2, arg1, arg1 // 10 // arg2, arg2, arg1,
                                                    arg1 // 10 // arg2 * arg2)
                               + step2_part3.format(arg1 // 10 // arg2 * arg2, arg1 // 10,
                                                    arg1 // 10 - arg1 // 10 // arg2 * arg2,
                                                    arg1 // 10 // arg2, arg2, arg1, arg1 // 10 // arg2 * arg2,
                                                    arg1 // 10 // arg2, arg2, arg1,
                                                    arg1 // 10 // arg2 * arg2, arg1 // 10 - arg1 // 10 // arg2 * arg2)
...
\:\:\:\longrightarrow    \:\:
\begin{{aligned}}
&\underline{{\text{{ }}\:00{}{{\color {{Salmon}} {}}}\:\:\small\:\:\:}}\\[-5pt]
{}&\big){}\\[-4pt]
-&{{\:\:\:\:{}\:\:}}\\[-4pt]
\hline
\hphantom{{ \big)}}&{{\:\:\:\:{}\:\:}}\\[-4pt]
...
"""
            ...
            step3_part3 = r"""
**Step 3.3:** And we subtract ${}$ from {} and then write ${}$.
...
$$
            """
            step3_statement = (step3_part1.format(arg2, arg1 - arg1 // 10 // arg2 * arg2 * 10,
                                                  (arg1 - arg1 // 10 // arg2 * arg2 * 10) // arg2,
                                                  arg1 // 10 // arg2, arg2, arg1, arg1 // 10 // arg2 * arg2,
                                                  arg1 // 10 - arg1 // 10 // arg2 * arg2,
                                                  arg1 // 10 // arg2, (arg1 - arg1 // 10 // arg2 * arg2 * 10) // arg2,
                                                  arg2, arg1, arg1 // 10 // arg2 * arg2,
                                                  arg1 - arg1 // 10 // arg2 * arg2 * 10)
                               + step3_part2.format((arg1 - arg1 // 10 // arg2 * arg2 * 10) // arg2, arg2,
                                                    arg1 // arg2 % 10 * arg2, arg1 // arg2 % 10 * arg2,
                                                    arg1 - arg1 // 10 // arg2 * arg2 * 10,
                                                    arg1 // arg2, arg2, arg1, arg1 // 10 // arg2 * arg2,
                                                    arg1 - arg1 // 10 // arg2 * arg2 * 10, arg1 // arg2,
                                                    arg2, arg1, arg1 // 10 // arg2 * arg2,
                                                    arg1 - arg1 // 10 // arg2 * arg2 * 10, arg1 // arg2 % 10 * arg2)
                               + step3_part3.format(arg1 // arg2 % 10 * arg2, arg1 - arg1 // 10 // arg2 * arg2 * 10,
                                                    arg1 % arg2,
                                                    arg1 // arg2, arg2, arg1, arg1 // 10 // arg2 * arg2,
                                                    arg1 - arg1 // 10 // arg2 * arg2 * 10, arg1 // arg2 % 10 * arg2,
                                                    arg1 // arg2, arg2, arg1, arg1 // 10 // arg2 * arg2,
...
\end{{aligned}}   
$$
""".format(arg1 // arg2, arg2, arg1, arg1 // 10 // arg2 * arg2, arg1 - arg1 // 10 // arg2 * arg2 * 10,
           arg1 // arg2 % 10 * arg2, arg1 % arg2)
...
\:\:\:\longrightarrow    \:\:
\begin{{aligned}}
&\underline{{\text{{ }}\:000{{\color {{Salmon}}{}}}\text{{ }}\text{{ }}\:\:\small\:\:\:}}\\[-5pt]
{}&\big){}\\[-4pt]
\end{{aligned}}
$$
"""
...
\:\:\:\longrightarrow    \:\:
\begin{{aligned}}
&\underline{{\text{{ }}\:000{}\text{{ }}\:\:\small\:\:\:}}\\[-5pt]
{}&\big){}\\[-4pt]
-&{{\:\:{{\color {{Salmon}}{}}}\:\:}}\\[-4pt]
\hline
\end{{aligned}}
...
$$
        """
            step3_statement = (step3_part1.format(arg2, arg1, arg1 // arg2,
                                                  arg2, arg1, arg1 // arg2, arg2, arg1)
                               + step3_part2.format(arg1 // arg2, arg2, arg1 // arg2 * arg2, arg1 // arg2 * arg2, arg1,
                                                    arg1 // arg2, arg2, arg1, arg1 // arg2, arg2, arg1,
                                                    arg1 // arg2 * arg2)
                               + step3_part3.format(arg1 // arg2 * arg2, arg1, arg1 % arg2,
                                                    arg1 // arg2, arg2, arg1, arg1 // arg2, arg2, arg1,
...
\end{{aligned}}  
$$
""".format(arg1 // arg2, arg2, arg1, arg1 // arg2 * arg2, arg1 % arg2)
...
""".format(arg1 * 10 // arg2, arg2, arg1, arg1 * 10 // arg2 * arg2 // 10, arg1 * 10 // arg2 * arg2 % 10,
           arg1 * 10 % arg2, arg1 / arg2, arg1 / arg2)

            step4_statement = ""

        elif decimal_digits == 2:
            step3_statement = r"""
**Step 3:** Since we obtained a non-zero remainder in the previous step, the calculation isn't complete. Because the remainder {} can't be divided evenly by {}, we need to form a new number to continue the division process. To do this, we take the last digit of the dividend, which is 0, and write it next to the {}, starting to work with this {}.
$$
\begin{{aligned}}
&\underline{{\:\,0.{{ {} }}\:}}\\[-5pt]
{}&\big){}\:0\\[-4pt]
...
\:\:\:\longrightarrow    \:\:
\begin{{aligned}}
&\underline{{\text{{ }}\,0.{{ {} }} \text{{ }}\text{{ }}}}\\[-5pt]
{}&\big){}\:0{{\color {{Salmon}} 0}}\\[-4pt]
&{{\:\:{{ {} }}\:{{ {} }}\!\:\small{{\downarrow}}\:}}\\[-4pt]
\hline
&\text{{ }}\:\:\:\:{{ {} }}{{\color {{Salmon}} 0}}\:\\[-4pt]
\end{{aligned}}
$$
*Note: In step 1, we attempted to divide ${{{}}}$ by {}. During this process, we encountered a remainder of {}. To continue our division, we appended a zero to the ${{ {} }}$ (noting that adding a zero after a decimal doesn't change the value of the number itself). We then brought this zero down and placed it after the {}, turning it into {}. This allowed us to continue the division until there was no remainder.*
""".format(arg1 * 10 % arg2, arg2, arg1 * 10 % arg2, arg1 * 10 % arg2 * 10,
           arg1 * 10 // arg2, arg2, arg1, arg1 * 10 // arg2 * arg2 // 10, arg1 * 10 // arg2 * arg2 % 10,
           arg1 * 10 % arg2, arg1 * 10 // arg2, arg2, arg1, arg1 * 10 // arg2 * arg2 // 10,
           arg1 * 10 // arg2 * arg2 % 10, arg1 * 10 % arg2,
           arg1, arg2, arg1 * 10 % arg2, arg1, arg1 * 10 % arg2, arg1 * 10 % arg2 * 10)

            step4_statement = r"""
**Step 4:** How many times does {} go into {}? ${}\times{} = {}$, we place the number {} in the quotient, next to the {}. We then write {} below the dividend and subtract to get the remainder, which is 0. Since we have no leftovers, we have finished the calculation.
$$
\begin{{aligned}}
&\underline{{\text{{ }}\,{} \text{{ }}\text{{ }}}}\\[-5pt]
{}&\big){}\:00\\[-4pt]
&{{\:\:{}\:{}\!\:\small\:}}\\[-4pt]
\hline
&\text{{ }}\:\:\:\:{}\:\\[-4pt]
...
\:\:\:\longrightarrow    \:\:
\begin{{aligned}}
&\underline{{\text{{ }}\,{}{{\color {{Salmon}} {}}}\text{{ }}}}\\[-5pt]
{}&\big){}\:00\\[-4pt]
&{{\:\:{}\:{}\:\:\:\:}}\\[-4pt]
\hline
&\text{{ }}\:\:\:\:{}\:\\[-4pt]
...
{}
$$
Therefore, the final solution is {}.""".format(arg2, arg1 * 10 % arg2 * 10, arg2, arg1 * 10 % arg2 * 10 // arg2,
                                               arg1 * 10 % arg2 * 10,
                                               arg1 * 10 % arg2 * 10 // arg2, arg1 * 10 // arg2, arg1 * 10 % arg2 * 10,
                                               arg1 * 10 // arg2 / 10, arg2, arg1, arg1 * 10 // arg2 * arg2 // 10,
                                               arg1 * 10 // arg2 * arg2 % 10, arg1 * 10 % arg2 * 10,
                                               arg1 * 10 // arg2 / 10, arg1 * 10 % arg2 * 10 // arg2, arg2, arg1,
                                               arg1 * 10 // arg2 * arg2 // 10, arg1 * 10 // arg2 * arg2 % 10,
                                               arg1 * 10 % arg2 * 10, arg1 * 10 % arg2 * 10, arg1 / arg2, arg1 / arg2)
        else:
...
class KnowledgePoint49Solution(SolutionGenerator):
    def get_solution(self, template: str, args: dict) -> str:
        ...
        def simplified_statement(arg1, arg2, arg3, gcd):
            if is_lowest_term(arg2 + arg3, arg1):
                step3_statement = r"""**Step 3:** Always reduce your final answer to its **lowest term**. And $\large\frac{{{}}}{{{}}}$ is already the lowest term.
                """.format(arg2 + arg3, arg1)
            else:
                step3_statement = r"""**Step 3:** Always reduce your final answer to its **lowest term**. In this case, the fraction $\large\frac{{{}}}{{{}}}$ can be simplified. Both {} and {} are divisible by {}. When we divide both {} and {} by their greatest common divisor {}, we get {} and {} respectively.
    $$
    \frac{{{}}}{{{}}} + \frac{{{}}}{{{}}} = \frac{{{}}}{{{}}}=
    ...
    $$
        ...
        def improper_fraction_statement(arg1, arg2, arg3, gcd):
            if arg2 + arg3 > arg1:
                step4_statement = r"""**Step 4:** Since our answer in the previous step is in the form of an improper fraction, we can further simplify it and convert it into a mixed number.
    $$
    \begin{{aligned}}
    ...
    $$
    Therefore, the final solution is ${}\large\frac{{{}}}{{{}}}$.""".format(
...
class KnowledgePoint53Solution(SolutionGenerator):
    def get_solution(self, template: str, args: dict) -> str:
        ...
        def getPrimeFactorization(num):
            ...
            if factors[0] == num:
                return "    - {} is prime. So it is the only factor".format(num)
            res = (
                "	- Start from small, the smallest number that can divide {} is {}, so we write down {}. Then ${} / {} = {}$.\n"
                .format(num, factors[0], factors[0], num, factors[0], num // factors[0]))
            num //= factors[0]
            for idx in range(1, len(factors) - 1):
                res += (
                    "	- Now, the smallest number that can divide {} is {}, so we write down {}. Then ${} / {} = {}$.\n"
                    .format(num, factors[idx], factors[idx], num, factors[idx], num // factors[idx]))
                num //= factors[idx]
            res += ("	- Notice {} is also a prime number, so we write down {}.\n".format(factors[len(factors) - 1],
                                                                                            factors[len(factors) - 1]))
            factorstr = str(factors)
            res += ("	- So, the prime factors of {} are {} and {}. (${} = "
                    .format(og, factorstr[1:len(factorstr) - 3], factorstr[len(factorstr) - 2:len(factorstr) - 1], og))
        ...
        def primeFactors(arg1, arg2, usage):
            ...
            if usage == 'Factor/Factors':
                ...
                if len(sharedfactors) == 0:
                    res += "-   There are no common prime factors of {} and {}.\n".format(arg1, arg2)
                elif len(sharedfactors) == 1:
                    res += "-   The common prime factor of {} and {} is {}.\n".format(arg1, arg2, sharedfactors[0])
                else:
                    res += "-   The common prime factors of {} and {} are ".format(arg1, arg2, sharedfactors[0])
                    res += "{}".format(sharedfactors[0])
                    for factor in sharedfactors[1:len(sharedfactors) - 1]:
                        res += ", {}".format(factor)
                    res += ", and {}".format(sharedfactors[len(sharedfactors) - 1])
                if len(uniquefactors) == 0:
                    res += "-   There are no unique prime factors of {} and {}.\n".format(arg1, arg2)
                elif len(uniquefactors) == 1:
                    res += "-   The unique prime factor of {} and {} is {}.\n".format(arg1, arg2, uniquefactors[0])
                else:
                    res += "-   The unique prime factors of {} and {} are ".format(arg1, arg2, uniquefactors[0])
                    res += "{}".format(uniquefactors[0])
                    for factor in uniquefactors[1:len(uniquefactors) - 1]:
                        res += ", {}".format(factor)
                    res += ", and {}".format(uniquefactors[len(uniquefactors) - 1])
                return res
            if usage == 'Chain':
                res = ""
...
class KnowledgePoint54Solution(SolutionGenerator):
    def get_solution(self, template: str, args: dict) -> str:
        ...
        def commonPrimeFactors(arg1, arg2, usage):
            ...
            if usage == 'List':
                if len(sharedfactors) == 1:
                    return "is {}".format(sharedfactors[0])
                res = "are {}".format(sharedfactors[0])
                for factor in sharedfactors[1:]:
                    res += " and {}".format(factor)
                return res
            if usage == 'Chain':
                if len(sharedfactors) == 1:
...
class KnowledgePoint55Solution(SolutionGenerator):
    def get_solution(self, template: str, args: dict) -> str:
        def LCMFraction(denominator, lcm):
            if denominator == lcm:
                return ""
            return " \\times \\frac{{{}}}{{{}}}".format(lcm // denominator, lcm // denominator)
        ...
        def simplify(numerator, denominator):
            ...
            if gcd_val == 1:
                if numerator < denominator:
                    return "The result cannot be simplified."
                else:
                    numerator //= gcd_val
                    denominator //= gcd_val
                    return (
        ...
        def simplifyAlgebra(numerator, denominator):
            ...
            denominator //= gcd_val
            if numerator > denominator:
                res += (" = {{\\color {{Salmon}}{}}}\\frac{{\\color {{Salmon}}{}}}{{\\color {{Salmon}}{}}}"
                        .format(numerator // denominator, numerator - numerator // denominator * denominator,
                                denominator))
            return res
...
class KnowledgePoint56Solution(SolutionGenerator):
    def get_solution(self, template: str, args: dict) -> str:
        ...
        def simplify(numerator, denominator):
            ...
            if gcd_val == 1:
                if numerator < denominator:
                    return "The result cannot be simplified further."
                else:
                    numerator //= gcd_val
                    denominator //= gcd_val
                    return (
                        ...
                        .format(numerator // denominator, numerator - numerator // denominator * denominator,
                                denominator))
            res = (
                "The result cannot be simplified. When we divide both {} and {} by their greatest common divisor {}, we get {} and {}, respectively."
                .format(numerator, denominator, gcd_val, numerator // gcd_val, denominator // gcd_val))
            if numerator > denominator:
                numerator //= gcd_val
                denominator //= gcd_val
    ...
    def get_solution(self, template: str, args: dict) -> str:
        def getVals(arg1, arg2, arg3, arg4, arg5, arg6, justFrac):
            ...
            if not justFrac:
                arg1 *= denominator  # 8
                arg4 *= denominator  # 4
                numerator = arg1 + arg4 + arg2 * arg6 + arg3 * arg5  # 8 + 4 + 1 * 2 + 1 * 2
            gcd_val = gcd(numerator, denominator)
            numerator //= gcd_val
            denominator //= gcd_val
            if denominator < 0:
            ...
            if numerator < 0:
                whole_num = -whole_num
            # print(numerator, denominator, whole_num, 'first')
            ...
            if denominator == 1:
                numerator = 0
            # print(numerator, denominator, whole_num, 'second')
            return numerator, denominator, whole_num

        def createText(numerator, denominator, whole_num):
            if denominator == 1:
                return "{}".format(numerator + whole_num)
            if whole_num == 0:
                if numerator < 0:
                    return "-\\frac{{{}}}{{{}}}".format(abs(numerator), denominator)
                return "\\frac{{{}}}{{{}}}".format(abs(numerator), denominator)
            return "{}\\frac{{{}}}{{{}}}".format(whole_num, abs(numerator), denominator)
        ...
        def getAnswerFracAndSign(arg1, arg2, arg3, arg4, arg5, arg6):
            ...
            if ansFrac[0] == '-':
                return "- {}".format(ansFrac[1:])
            return "+ {}".format(ansFrac)
    ...
    def get_solution(self, template: str, args: dict) -> str:
        ...
        def getColor(num):
            if isinstance(num, int):
                return "{\\color{Salmon}" + str(num) + '}'
            num = str(num)
            idx = num.index('.')
            return num[:idx + 1] + "{\\color{Salmon}" + num[idx + 1:] + "}"
...
class KnowledgePoint59Solution(SolutionGenerator):
    def get_solution(self, template: str, args: dict) -> str:
        ...
        # i need another string to get the actual division image bc carry changes every loop and id be regenerating it <-- (maybe)
        def getSteps(arg1, arg2):
            ...
            for step in range(3, (len(str(arg1)) + 2)):
                ...
                if step == 3:
                    res += " Start with {} divided by {}, ".format(arg1str[step - 3], arg2)
                else:
                    res += " Bring down {} ".format(arg1str[step - 3])
                if int(arg1str[step - 3]) + carry * 10 < arg1:
                    res += "but {} does not divide {} so we write zero, and move to the right.\n$$\n".format(arg2,
                                                                                                             arg1str[
                                                                                                                 step - 3])
                    # oldcarry = carry
                    carry *= 10
                    ...
                    res += "so we get {} divided by {}".format(carry * 10 + int(arg1str[step - 3]), arg2)
                    if carry > 0:
                        res += " because we had a remainder in the prior step"
                    res += (". {} divided by {} equals {}, so we write that down.\n$$\n"
                            .format(carry * 10 + int(arg1str[step - 3]), arg2,
                                    (carry * 10 + int(arg1str[step - 3])) // arg2))
                    oldcarry = carry
...
class KnowledgePoint60Solution(SolutionGenerator):
    def get_solution(self, template: str, args: dict) -> str:
        ...
        # python precision messes up some answers
        def getAnswer(arg1, arg2):
...
class KnowledgePoint64Solution(SolutionGenerator):
    def get_solution(self, template: str, args: dict) -> str:
        ...
        def getNumbers(arg1):
            nums = arg1.split(",")
            if len(nums) > 1:
                return "are {} numbers".format(len(nums))
            return "is 1 number"
        ...
        def mean(arg1, usage=1):
            ...
            nums = [int(num) for num in lst]
            if usage == 1:
                return "{} \\div {} = {}".format(sum(nums), len(nums), sum(nums) // len(nums))
...
class KnowledgePoint65Solution(SolutionGenerator):
    # I checked all the possible questions, none of them ask for the median of a list with an even number of ints
    # We should add a scenario where we teach them to find the two middle numbers and find the mean
    # This code will then be more complicated but it would just be adding a second function for the even length scenario
    def get_solution(self, template: str, args: dict) -> str:
        args['arg1'] = ",".join(args['arg1'])
        def getList(arg1):
            nums = arg1.split(",")
        ...
        def getPositionStatement(arg1):
            ...
            lst = [int(num) for num in nums]
            if len(lst) % 2 == 1:
                return "In this case, the middle number is at position {}.".format(len(lst) // 2 + 1)
            return "In this case, the middle numbers are at positions {} and {}.".format(len(lst) // 2,
                                                                                         len(lst) // 2 + 1)
        ...
        def getStatementForEven(arg1):
            ...
            if len(lst) % 2 == 1:
                return ""
            return ("Because the list is of even length, we must take the mean of the two middle numbers, {} and {}. "
                    "The mean of {} and {} is {}.".format(
...
class KnowledgePoint66Solution(SolutionGenerator):
    def get_solution(self, template: str, args: dict) -> str:
        ...
        def getHighlightedOccurences(arg1):
            ...
            for num in lst[:len(lst) - 1]:
                if num == val:
                    res += "{\\color{salmon}" + str(num) + "}, "
                else:
                    res += str(num) + ", "
            if lst[-1] == val:
                res += "{\\color{salmon}" + str(val) + "}"
            else:
                res += str(lst[-1])
            return res
...
class KnowledgePoint68Solution(SolutionGenerator):
    def get_solution(self, template: str, args: dict) -> str:
        ...
        def getSteps(arg1):
            ...
            res += "**Step 1:** Based on the **Order of Operations**, we evaluate the equation inside the parenthesis first.".format(
                step)
            if parenthesisOperations == 0:
                res += "In this case, there are no parenthesis.\n\n"
                step += 1
            else:
                step += 1
                res += "\n$$\n"
                res += getExpression(parenthesis) + "\n$$\n"
                res += "**Step {}:** We now check for exponents within the parenthesis. ".format(step)
                count = parenthesis.count('^')
                if count == 0:
                    res += "In this case there are no exponents within the parenthesis.\n\n"
                else:
                    if count == 1:
                        res += "In this case there is 1 exponent within the parenthesis. "
                    else:
                        res += "In this case there are {} exponents within the parenthesis. ".format(count)
                    res += "\n$$\n" + getExpression(arg1)
                    res += " = "
                ...
                step += 1
                res += "**Step {}:** We now check for multiplication and division operations within the parenthesis. ".format(
                    step)
                countm, countd = parenthesis.count('*'), parenthesis.count('/')
                if countm + countd == 0:
                    res += "In this case there are no multiplication or division operations within the parenthesis.\n\n"
                else:
                    res += "In this case there are {} multiplication and {} division operations within the parenthesis and we go from left to right.".format(
                        countm, countd)
                    res += "\n$$\n" + getExpression(arg1)
                    res += " = "
                    oldparenthesis = parenthesis
                    ...
                    res += getExpression(arg1) + "\n$$\n"
                step += 1
                res += "**Step {}:** We now check for addition and subtraction operations within the parenthesis. ".format(
                    step)
                counta, counts = parenthesis.count('+'), parenthesis.count('-')
                if counta + counts == 0:
                    res += "In this case there are no addition or subtraction operations within the parenthesis.\n\n"
                else:
                    res += "In this case there are {} addition and {} subtraction operations within the parenthesis and we go from left to right.".format(
                        counta, counts)
                    res += "\n$$\n" + getExpression(arg1)
                    res += " = "
                    oldparenthesis = parenthesis
                ...
                if '(' in arg1:
                    ...
                    arg1 = arg1.replace(')', '')
                step += 1
            res += "**Step {}**: Now that we've finished the operations within the parenthesis, we do operations outside of the parenthesis. Starting with exponents. ".format(
                step)
            count = arg1.count('^')
            if count == 0:
                res += "In this case there are no exponents left.\n"
            else:
                if count == 1:
                    res += "In this case there is 1 exponent operation left."
                else:
                    res += "In this case there are {} exponent operations left.".format(count)
                res += "\n$$\n" + getExpression(arg1)
                res += " = "
                for iter in range(count):
                    idx = arg1.index('^')
                ...
                res += getExpression(arg1) + "\n$$\n"
            step += 1
            res += "**Step {}:** We now check for multiplication and division operations. ".format(step)
            countm, countd = arg1.count('*'), arg1.count('/')
            if countm + countd == 0:
                res += "In this case there are no multiplication or division operations left.\n\n"
            else:
                res += "In this case there are {} multiplication and {} division operations and we go from left to right.".format(
                    countm, countd)
                res += "\n$$\n" + getExpression(arg1)
                res += " = "
                for iter in range(countm + countd):
                ...
                res += getExpression(arg1) + "\n$$\n"
            step += 1
            res += "**Step {}:** We now check for addition and subtraction operations. ".format(step)
            counta, counts = arg1.count('+'), arg1.count('-')
            if counta + counts == 0:
                res += "In this case there are no addition or subtraction operations left.\n\n"
            else:
                res += "In this case there are {} addition and {} subtraction operations and we go from left to right.".format(
                    counta, counts)
                res += "\n$$\n" + getExpression(arg1)
                res += " = "
                for iter in range(counta + counts):
            ...
            # copy and paste in the same way as ^ for */ and +-
    ...
    def get_solution(self, template: str, args: dict) -> str:
        ...
        def getEquation(arg1, arg2, arg3, arg4, arg5, arg6, sign1, sign2, convert=False):
            ...
            if arg6 == 1:
                frac3 = arg5
            return ("{} {} {} {} {}"
                    .format(frac1, sign1, frac2, sign2, frac3))
            ...
            for i in range(6):
                if gcds[i] != 1:
                    ...
                    if d == 'first':
                        ...
                        arg6 = arg6 // gcds[i]
                    self.step += 1
            res += "**Step {}:** Now all fractions have been simplified to their lowest terms, multiply them.\n$$\n".format(
                self.step)
            res += getEquation(arg1, arg2, arg3, arg4, arg5, arg6, sign1, sign2, True) + " = "
...
class KnowledgePoint72Solution(SolutionGenerator):
    def get_solution(self, template: str, args: dict) -> str:
        ...
        def getExpression(arg1):
            ...
            # ops = [op for op in arg1 if op in ['-', '*', '+', '/', '^', '(', ')']] #does not work for middle steps in the getSteps function
            ops = []
            withinbraces = False
            for char in arg1:
                if char == '[':
        ...
        def getSteps(arg1):
            ...
            if parenthesisOperations == 0:
                ...
                res += "**Step {}:** We now check for exponents within the parenthesis. ".format(step)
                res += "In this case there are no exponents within the parenthesis.\n\n"
                # no need for exponents bc there are none in question bank
                # count = parenthesis.count('^')
                # if count == 0:
                #     res += "In this case there are no exponents within the parenthesis.\n\n"
                # else:
                ...
                #         subequation = parenthesis[lower+1:higher]
                #         idx = subequation.index('^')
                #         parenthesis = parenthesis.replace(subequation, str(int(subequation[0:idx])**int(subequation[idx+1:len(subequation)])), 1)
                #     arg1 = arg1.replace(oldparenthesis, parenthesis)
                #     res += getExpression(arg1) + "\n$$\n" 
                step += 1
                res += "**Step {}:** We now check for multiplication and division operations within the parenthesis. ".format(
                ...
                if countm + countd == 0:
                    ...
                    res += "In this case there are {} multiplication and {} division operations within the parenthesis and we go from left to right. Simplify at the end if possible. ".format(
                        countm, countd)
                    res += "\n$$\n" + getExpression(arg1)
                    res += " = "
                    oldparenthesis = parenthesis
                ...
                if counta + counts == 0:
                    ...
                    res += "In this case there are {} addition and {} subtraction operations within the parenthesis and we go from left to right. Simplify at the end if possible.".format(
                        counta, counts)
                    res += "\n$$\n" + getExpression(arg1)
                    res += " = "
                    oldparenthesis = parenthesis
                    ...
                    arg1 = arg1.replace('(', '')
                    arg1 = arg1.replace(')', '')
                    res += getExpression(middle) + " = "
                    res += getExpression(middle2) + " = "
                    if needtosimplify:
            ...
            #        subequation = arg1[lower+1:higher]
            #        idx = subequation.index('^')
            #        arg1 = arg1.replace(subequation, str(int(subequation[0:idx])**int(subequation[idx+1:len(subequation)])), 1)
            #    res += getExpression(arg1) + "\n$$\n"
            step += 1
            res += "**Step {}:** We now check for multiplication and division operations. ".format(step)
            countm, countd = arg1.count('*'), arg1.count('/')
            if countm + countd == 0:
                ...
                res += "In this case there are {} multiplication and {} division operations and we go from left to right. Simplify at the end if possible.".format(
                    countm, countd)
                res += "\n$$\n" + getExpression(arg1)
                res += " = "
                ...
                for iter in range(countm + countd):
                    ...
                    if idx == idxm:
                        ...
                        arg1 = arg1.replace(subequation, replace, 1)
                if needtosimplify:
                    res += getExpression(unsimplified) + " = "
                    res += getExpression(simplifystatement) + " = "
                res += getExpression(arg1) + "\n$$\n"
            step += 1
            res += "**Step {}:** We now check for addition and subtraction operations. ".format(step)
            counta, counts = arg1.count('+'), arg1.count('-')
            if counta + counts == 0:
                ...
                res += getExpression(arg1) + "\n$$\n"
            step += 1
            res += "**Step {}:** Simplify if possible. ".format(step)
            numerator, denominator = getVals(arg1)
            numerator = int(numerator)
            denominator = int(denominator)
            gcd_val = gcd(numerator, denominator)
            if gcd_val == 1:
                res += "In this case, the fraction cannot be further simplified.\n\n"
...
class KnowledgePoint73Solution(SolutionGenerator):
    def get_solution(self, template: str, args: dict) -> str:
        ...
        def getSteps(arg1):
            res = ""
            res += "**Step 1:** First convert all mixed numbers into improper fractions. "
            if '<' not in arg1:
                res += "In this case, there are no mixed numbers.\n\n"
            else:
                res += "\n$$\n" + getExpression(arg1) + " = "
                nums = re.findall(r'\[.*?\]', arg1)
            ...
            if '(' in arg1:
                ...
                parenthesisOperations = parenthesis.count('+') + parenthesis.count('-') + parenthesis.count(
                    '*') + parenthesis.count('/') + parenthesis.count('^')
            res += "**Step 2:** Based on the **Order of Operations**, we evaluate the equation inside the parenthesis first. ".format(
                step)
            if parenthesisOperations == 0:
                res += "In this case, there are no parenthesis.\n\n"
                step += 1
            ...
            if counta + counts == 0:
                ...
                for iter in range(counta + counts):
                    ...
                    if idx == idxa:
                        ...
                        denominator1 = int(denominator1)
                        denominator2 = int(denominator2)
                        lcm_val = lcm(denominator1, denominator2)
                        factor1 = lcm_val // denominator1
                        factor2 = lcm_val // denominator2
                        replace1 = (
                            "[{}\\times{{\\color{{Salmon}}{{{}}}}}|{}\\times{{\\color{{Salmon}}{{{}}}}}]-[{}\\times{{\\color{{Salmon}}{{{}}}}}|{}\\times{{\\color{{Salmon}}{{{}}}}}]"
